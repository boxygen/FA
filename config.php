...
if (!isset($path_to_root) || isset($_GET['path_to_root']) || isset($_POST['path_to_root']))
	...
	//$error_logfile = '';
	$remote_installation = false; //added by faisal
	$remote_install_name = 'MAC';
	...
	if ($_SERVER["SERVER_NAME"]=='localhost')
		...
		list($subdomain,$host) = explode('.', $_SERVER["SERVER_NAME"]); //added by Faisal

	// $error_logfile = VARLIB_PATH.'/errors.log';
	$error_logfile = '';
	$debug 			= 1;	// show sql on database errors

	if ($subdomain=="debug" || $subdomain=="test")
...
if ($subdomain=="rdebug")
{
	...
	if ($subdomain=="pakerp")
		{
			$app_title = "PakERP";
			$power_by 		= "PakERP";
			$power_url 		= "https://PakERP.com";
			$help_base_url = 'https://pakerp.net/wiki/doc.php?id=Help.';
		}
	elseif ($subdomain=='erp')
		{
			if (file_exists($path_to_root."/seller/".$host.".png"))
			{
				$app_title = strtoupper($host);
				$power_by 		= strtoupper($host);
				$power_url 		= "#";
				$help_base_url = 'https://ourerp.services/wiki/doc.php?id=Help.';
			}
...

			{
				if (file_exists($path_to_root."/seller/".$host.".png"))
				{
					...
					$power_by 		= strtoupper($host);
					$power_url 		= "#";
					$help_base_url = 'https://'.$subdomain.'.ourerp.services/wiki/doc.php?id=Help.';
				}
	...
	@date_default_timezone_set('Asia/Karachi'); //added by faisal for timezone
	...
	// 	context help feed from Boxygen.com
...
*/
	$xr_provider_authoritative = true;
...
*/
	...
	$erp_blocked = false; //by faisal
	$erp_blocked_msg = "We are upgrading system. We are sorry for inconvenience";
	$erp_blocked_url = "";
	$allowed_coys = array('SAADTRADERS','Boxygen','MHA');

	// pos by anoop
	$allow_item_creation_in_pos=true;
$allow_customer_creation_in_pos=true;
$allow_salesman_selection_in_pos=true;
